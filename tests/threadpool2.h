...
struct thread_pool
{
        ...
        pthread_cond_t queued_cond;        // Conditional to wake workers up when new task is queued
        pthread_mutex_t queued_lock;       // Lock that goes together with queued conditional
        int worker_count;                  // Number of workers
        pthread_mutex_t worker_lock;       // Lock for worker arr, needed since threads are started while some threads are active
